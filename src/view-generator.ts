...
function buildProjectCards(projects: ProjectData[]): string {
  return projects.map((p) => {
    ...
    const sessionsHtml = p.sessions.slice(0, 3).map((s) => {
      ...
      return `
        <div class="session-item" data-title="${escapeHtml(s.title)}" data-request="${escapeHtml(s.userRequest)}" onclick="openSessionModal('${escapeHtml(p.name)}', '${escapeHtml(s.title)}')">
...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .session-modal-qa-item .qa-content code { background: var(--apple-gray-1); padding: 2px 6px; border-radius: 4px; font-family: monospace; font-size: 13px; }
    .session-modal-qa-item .qa-content pre { background: var(--apple-gray-1); padding: 16px; border-radius: 8px; overflow-x: auto; margin: 8px 0; }
    .session-modal-qa-item .qa-content pre code { background: none; padding: 0; }
    @media (max-width: 768px) {
